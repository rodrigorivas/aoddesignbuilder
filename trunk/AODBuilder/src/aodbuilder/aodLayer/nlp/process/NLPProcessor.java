package aodbuilder.aodLayer.nlp.process;
...
public class NLPProcessor {
	...
	private void loadParser() {
		...
		URL url = FileConstants.PARSER_ENGLISH_RESOURCE_URL;
		if (url==null){
			//get local resource URL
			url = ResourceLoader.getResourceURL(FileConstants.PARSER_ENGLISH);
		}
		try{
			loadFromURL(url);
		}catch (Exception e) {
	...
	private void loadDefault() {
		logger.info("Trying to load resource: "+FileConstants.PROPERTIES_PATH+FileConstants.PARSER_ENGLISH);
		lp = new LexicalizedParser(FileConstants.PROPERTIES_PATH+FileConstants.PARSER_ENGLISH);
		logger.info("Parser loaded from default file on "+FileConstants.PROPERTIES_PATH+FileConstants.PARSER_ENGLISH);
	}
